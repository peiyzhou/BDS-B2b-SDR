...
% pseudoranges, find receiver position, and generate precise ephemeris 
% in Sp3 format.
%
% 5) Plot the results.
%% Initialization =========================================================
...
if (fid > 0)
    ...
    [navResults,eph,ephPPP] = postNavigation(trkResults, settings);
    ...
    %% Generate Sp3 file
    if settings.PPP_B2b &&~isempty(settings.Sp3file)
        genSp3(fix(min(navResults.transmitTime,[],"all")),fix(max(navResults.transmitTime,[],"all")),eph,ephPPP,settings)
    end
